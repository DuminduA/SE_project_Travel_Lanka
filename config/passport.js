/**
 * Created by dumindu on 27/04/2017.
 */
...
}); // how to store the user in the session. store the user using id
...
passport.use('local.signup', new LocalStrategy({
    ...
    if(usertype == 'blank'){return done(null,false,{message:'please choose a user type'});}
...
passport.use('local.login', new LocalStrategy({
    ...
    if(usertype == 'blank'){return done(null,false,{message:'please choose a user type'});}
        ...
        tourist.findOne({'email':email},function(err, user){
            if(err){ return done(err)}
            if(!user){ return done(null, false , {message:'You have to sign up first'});} // no error but still does not successful
